...
interface CreatePollFormProps {
  setPolls: (arg: SetStateAction<Poll[]>) => void
}

export function CreatePollForm({ setPolls }: CreatePollFormProps) {
  const mutation = useMutation<CreatePollResponse, Error, CreatePollRequest>({
    ...
    onSuccess: (response, request) => {
      setPolls((prev) => [{ id: response.data.pollId, title: request.title }, ...prev])
    },
  })
